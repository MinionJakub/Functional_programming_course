...
let append a b = let* c = flip in if c then a else   b;;

let rec match_regexp (reg : 'a regexp) (xs : 'a list) : 'a list option BT.t = 
  match reg with
  | Eps -> return None
  | Lit p -> begin match xs with | [] -> fail | x::xs -> if p x then return (Some xs) else fail end
  | Or(a,b) -> append (match_regexp a xs) (match_regexp b xs)
  | Cat(a,b) -> begin 
  ...
  | Star a ->  
    let* suf = match_regexp (Or(a,Eps)) xs in begin
    match suf with 
    | None -> return None
