...
let rec traverse_down ctx node =
  ...
  | Theorem _ -> failwith "Proofed already"
...
let intro name pf =
  ...
  | Incomplete (ctx, (formula, named_ass)) -> 
    ...
    | _ -> failwith "This is not implication" end
  | _ -> failwith "Proofed already"
...
let helpCreate formula proof =
  match proof with 
  | Complete(_) -> failwith "It should be unproofed"
