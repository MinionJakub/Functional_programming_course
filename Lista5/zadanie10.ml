...
let list_to_fun lista =
  fun x -> match List.find_opt (fun y -> (fst y) == x) lista with
  | None -> failwith "Unknown value"
  | Some a -> snd a;;
  
let fun_gen sq1 sq2 = 
  let value = (Seq.map List.of_seq (gen_func sq1 sq2)) in
  Seq.fold_left (fun x y -> Seq.cons (list_to_fun y) x) Seq.empty value;;
